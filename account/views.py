...
def activate_account(request, uidb64, token): # type: ignore
    try:
        ...
        if user.is_active:
            messages.warning(request, "This account has already been activated.") # type: ignore
            return redirect('account:login')
            
        if default_token_generator.check_token(user, token):
            user.is_active = True       # Activate the account
            user.save()                 # Save the updated user
            messages.success(request, "Your account has been activated successfully!") # type: ignore
            return redirect('login')
        else:
            messages.error(request, "The activation link is invalid or has expired.") # type: ignore
            return redirect('login')
